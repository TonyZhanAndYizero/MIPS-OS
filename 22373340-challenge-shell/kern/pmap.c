...
void page_init(void) {
	...
	u_long under_freemem_npage = (freemem - KSEG0) / PAGE_SIZE;
	for(u_long i = 0; i < under_freemem_npage; i++){
		pages[i].pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for(u_long i = under_freemem_npage; i < npage; i++){
		pages[i].pp_ref = 0;
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	if((PTE_FLAGS(*pgdir_entryp) & PTE_V) == 0){
		if(create == 1){
			...
			pp->pp_ref++;
			*pgdir_entryp = PTE_ADDR(page2pa(pp)) | PTE_C_CACHEABLE | PTE_V;
		} else {
			*ppte = NULL;
			return 0;
		}
